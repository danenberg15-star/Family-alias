...
export default function SetupStep(props: SetupStepProps) {
  ...
  const shareWhatsApp = () => {
    const text = `בואו לשחק אליאס! כנסו לחדר: ${props.roomId}\nhttps://family-alias.vercel.app/`;
  ...
  return (
    <div style={styles.flexLayout} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
      {draggedPlayer && (
        <div style={{ ...styles.draggingGhost, left: ghostPos.x - 60, top: ghostPos.y - 25 }}>{draggedPlayer.name}</div>
      )}

      {/* כותרת ואייקון שיתוף */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', paddingTop: '10px' }}>
        <h1 style={{ ...styles.entryTitle, fontSize: '2.8rem', margin: 0, letterSpacing: '2px' }}>{props.roomId}</h1>
      ...
      <div style={{ width: '100%', marginTop: '10px' }}>
        {/* בחירת סוג משחק - עיצוב מסך ראשון */}
        <div style={styles.setupSelectionRow}>
          <button 
            onClick={() => props.setGameMode("individual")} 
            style={{ ...styles.setupSelectionBtn, ...(props.gameMode === "individual" ? styles.setupSelectionBtnActive : {}) }}
          >
            יחידים
          </button>
          <button 
            onClick={() => props.setGameMode("team")} 
            style={{ ...styles.setupSelectionBtn, ...(props.gameMode === "team" ? styles.setupSelectionBtnActive : {}) }}
          >
            קבוצות
          </button>
        </div>

        {/* בחירת רמת קושי - עיצוב מסך ראשון */}
        <div style={styles.setupSelectionRow}>
          <button 
            onClick={() => props.setDifficulty("age-appropriate")} 
            style={{ ...styles.setupSelectionBtn, ...(props.difficulty === "age-appropriate" ? styles.setupSelectionBtnActive : {}) }}
          >
            מותאם גיל
          </button>
          <button 
            onClick={() => props.setDifficulty("easy")} 
            style={{ ...styles.setupSelectionBtn, ...(props.difficulty === "easy" ? styles.setupSelectionBtnActive : {}) }}
          >
            רמה קלה
          </button>
        </div>
      ...
      <div style={{ ...styles.setupGrid, gridTemplateColumns: props.gameMode === "team" && props.numTeams === 2 ? '1fr 1fr' : 'repeat(auto-fit, minmax(130px, 1fr))' }}>
        {Array.from({ length: props.gameMode === "team" ? props.numTeams : 1 }).map((_, tIdx) => (
          <div key={tIdx} ref={el => { teamRefs.current[tIdx] = el; }} style={{ ...styles.teamBox, ...(hoveredTeam === tIdx ? styles.teamBoxGlowing : {}) }}>
            ...
            <div style={{ flex: 1 }}>
              {props.players.filter(p => props.gameMode === "individual" || p.teamIdx === tIdx).map(p => (
                <div key={p.id} onPointerDown={(e) => handlePointerDown(e, p)} style={{ ...styles.playerCard, opacity: draggedPlayer?.id === p.id ? 0 : 1 }}>{p.name}</div>
              ))}
            </div>
          </div>
        ))}
