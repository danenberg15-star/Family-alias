...
export default function SetupStep(props: SetupStepProps) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    setHoveredTeam(found);
  };

  return (
    <div style={styles.flexLayout} onPointerMove={handlePointerMove} onPointerUp={() => { if (draggedPlayer && hoveredTeam !== null) props.onPlayerMove(draggedPlayer.id, hoveredTeam); setDraggedPlayer(null); setHoveredTeam(null); }}>
      ...
      <div style={{ ...styles.setupGrid, gridTemplateColumns: props.gameMode === "team" ? (props.numTeams === 2 ? '1fr 1fr' : 'repeat(3, 1fr)') : '1fr' }}>
        {Array.from({ length: props.gameMode === "team" ? props.numTeams : 1 }).map((_, tIdx) => (
          <div key={tIdx} ref={el => { teamRefs.current[tIdx] = el; }} style={{ ...styles.teamBox, ...(hoveredTeam === tIdx ? { borderColor: '#ffd700', backgroundColor: 'rgba(255,215,0,0.1)' } : {}) }}>
            {props.gameMode === "team" && props.numTeams > 2 && props.players.filter(p => p.teamIdx === tIdx).length === 0 && (
              <button onClick={() => props.setNumTeams(props.numTeams - 1)} style={{ position: 'absolute', top: '-5px', right: '-5px', width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#ef4444', border: 'none', color: 'white', fontWeight: 'bold' }}>-</button>
            )}
            ...
            {props.players.filter(p => props.gameMode === "individual" || p.teamIdx === tIdx).map(p => (
              <div key={p.id} onPointerDown={(e) => { setDraggedPlayer(p); setGhostPos({ x: e.clientX, y: e.clientY }); }} style={{ ...styles.playerCard, opacity: draggedPlayer?.id === p.id ? 0.3 : 1 }}>{p.name}</div>
            ))}
          </div>
        ))}
