...
interface SetupStepProps {
  ...
  updateTeamNames: (names: string[]) => void; onExit: () => void;
...
export default function SetupStep(props: SetupStepProps) {
  ...
  return (
    <div style={styles.flexLayout} onPointerMove={(e) => { if (draggedPlayer) setGhostPos({ x: e.clientX, y: e.clientY }); let found: number | null = null; for (let i = 0; i < (props.gameMode === "team" ? props.numTeams : 1); i++) { const rect = teamRefs.current[i]?.getBoundingClientRect(); if (rect && e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom) { found = i; break; } } setHoveredTeam(found); }} onPointerUp={() => { if (draggedPlayer && hoveredTeam !== null) props.onPlayerMove(draggedPlayer.id, hoveredTeam); setDraggedPlayer(null); setHoveredTeam(null); }}>
      ...
      <div style={{ ...styles.setupGrid, gridTemplateColumns: props.gameMode === "team" ? 'repeat(3, 1fr)' : '1fr' }}>
          ...
          return (
            <div key={tIdx} ref={el => { teamRefs.current[tIdx] = el; }} style={{ ...styles.teamBox, ...(hoveredTeam === tIdx ? { borderColor: '#ffd700', backgroundColor: 'rgba(255,215,0,0.1)' } : {}) }}>
              <div style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '4px', marginBottom: '8px', textAlign: 'center' }}>
                <span style={{ color: '#ffd700', fontWeight: 'bold', fontSize: '0.8rem' }}>
                  {props.gameMode === "team" ? props.teamNames[tIdx] : "משתתפים"}
                  {props.gameMode === "team" && <span onClick={() => props.editTeamName(tIdx)} style={{ cursor: 'pointer', fontSize: '0.8rem', opacity: 0.6, marginRight: '5px' }}>✎</span>}
                </span>
              </div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                {teamPlayers.length > 0 ? (
                  teamPlayers.map(p => (
                    <div key={p.id} onPointerDown={(e) => { setDraggedPlayer(p); setGhostPos({ x: e.clientX, y: e.clientY }); }} style={{ ...styles.playerCard, width: '100%', fontSize: '0.8rem', opacity: draggedPlayer?.id === p.id ? 0.3 : 1 }}>{p.name}</div>
                  ))
                ) : (
        ...
        {props.gameMode === "team" && props.numTeams < 6 && !hasEmptyTeam && (
