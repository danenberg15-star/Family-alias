...
export const styles: { [key: string]: CSSProperties } = {
  container: { 
    ...
    backgroundColor: '#05081c', direction: 'rtl', overflow: 'hidden', 
    position: 'fixed', touchAction: 'none', userSelect: 'none', overscrollBehavior: 'none' 
  },
  safeAreaWrapper: { 
    width: '100%', maxWidth: '450px', height: '100%', display: 'flex', 
    flexDirection: 'column', paddingTop: 'env(safe-area-inset-top, 20px)', 
    paddingBottom: 'env(safe-area-inset-bottom, 20px)', paddingLeft: '20px', 
    paddingRight: '20px', alignItems: 'center', position: 'relative', boxSizing: 'border-box' 
  },
  ...
  // --- מסך לובי (Lobby) - שחזור לפי התמונה ---
  lobbyTitle: { color: 'white', fontSize: '2rem', fontWeight: 'bold', marginTop: '40px', marginBottom: '30px', textAlign: 'center' },
  lobbyCenterArea: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '30px', width: '100%', flex: 1 },
  lobbyButtonWhite: { padding: '8px 20px', borderRadius: '8px', backgroundColor: 'white', color: 'black', fontWeight: 'bold', border: 'none', fontSize: '1rem', cursor: 'pointer' },
  lobbyJoinText: { fontSize: '1.4rem', fontWeight: 'bold', cursor: 'pointer', textAlign: 'center' },

  // --- מסך אפיון (Setup) - ביצועים גבוהים ---
  ...
  teamBox: { backgroundColor: 'rgba(255, 255, 255, 0.03)', borderRadius: '20px', border: '2px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', padding: '10px', overflow: 'hidden', transition: 'border 0.2s' },
  teamBoxGlowing: { borderColor: '#ffd700', backgroundColor: 'rgba(255, 215, 0, 0.15)', boxShadow: '0 0 15px rgba(255, 215, 0, 0.2)' },
  playerCard: { backgroundColor: 'rgba(255, 255, 255, 0.1)', color: 'white', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.15)', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', fontWeight: 'bold', margin: '4px 0', boxSizing: 'border-box' },
  goldButtonFixed: { width: '100%', minHeight: '65px', borderRadius: '20px', background: 'linear-gradient(135deg, #ffd700, #b8860b)', color: '#05081c', fontWeight: '900', border: 'none', fontSize: '1.5rem', marginTop: 'auto', boxShadow: '0 4px 15px rgba(255, 215, 0, 0.3)' },
