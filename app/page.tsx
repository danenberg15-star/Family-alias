...
export default function FamilyAliasApp() {
  ...
  const handleGuess = async (isSkip: boolean) => {
    ...
    if (!isSkip && activeHover && activeHover !== 'SKIP') {
      ...
      const newScore = (roomData.totalScores[entity] || 0) + 1;
      updates[`totalScores.${entity}`] = newScore;
      if (newScore >= 50) { updates.step = 7; updates.winner = entity; }
    }
  ...
  return (
    <div style={styles.container} onPointerMove={handlePointerMove} onPointerUp={() => {
      ...
      <div style={styles.safeAreaWrapper}>
        {step > 1 && <button onClick={handleExit} style={styles.exitBtn}>✕</button>}
