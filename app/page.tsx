  ...
  return (
    <div style={styles.container} onPointerMove={(e) => {
      ...
      <div style={styles.safeAreaWrapper}>
        ...
        {step === 1 && (
          <EntryStep onNext={(n, a, action, code) => { 
            setUserName(n); setUserAge(a); if (action === 'create') handleCreateRoom(); else if (action === 'join' && code) handleJoinRoom(code); 
          }} />
        )}
        ...
        {step === 3 && roomData && (
          <SetupStep 
            ...
            editTeamName={(idx: number) => { 
              const n = prompt("שם קבוצה חדש:", roomData.teamNames[idx]); 
        ...
        {step === 5 && roomData && (
          <GameStep 
            ...
            targets={gameTargets} targetsRef={targetsRef as any} 
            score={roomData.roundScore} onPause={() => updateRoom({ isPaused: true })} 
