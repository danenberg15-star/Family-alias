  ...
  return (
    <div style={styles.container} onPointerMove={(e) => {
      ...
      if (isDragging.current && activeHover) {
        const isSkip = activeHover === "SKIP";
        updateRoom({ roundScore: (roomData.roundScore || 0) + (isSkip ? -1 : 1), currentWordIdx: roomData.currentWordIdx + 1 });
      }
      ...
      <div style={styles.safeAreaWrapper}>
        ...
        {step === 3 && roomData && <SetupStep roomId={roomId!} gameMode={roomData.gameMode} setGameMode={(m) => updateRoom({ gameMode: m })} difficulty={roomData.difficulty || "age-appropriate"} setDifficulty={(d) => updateRoom({ difficulty: d })} numTeams={roomData.numTeams} setNumTeams={(n) => updateRoom({ numTeams: n })} players={roomData.players} teamNames={roomData.teamNames} onPlayerMove={(pId, tIdx) => { const p = roomData.players.map((pl:any) => pl.id === pId ? {...pl, teamIdx: tIdx} : pl); updateRoom({ players: p }); }} editTeamName={(idx) => { const n = prompt("שם קבוצה:", roomData.teamNames[idx]); if(n) { const t = [...roomData.teamNames]; t[idx] = n; updateRoom({ teamNames: t }); } }} onStart={() => updateRoom({ step: 4, preGameTimer: 3 })} onExit={handleFullReset} />}
        ...
        {step === 5 && roomData && ( isIDescriber ? ( <GameStep timeLeft={roomData.timeLeft} currentWord={roomData.shuffledWords[roomData.currentWordIdx % roomData.shuffledWords.length]} wordRef={wordRef} skipRef={skipRef} isDraggingWord={isDraggingWord} onPointerDown={() => { isDragging.current = true; setIsDraggingWord(true); }} targets={gameTargets} targetsRef={targetsRef as any} score={roomData.roundScore} onPause={() => updateRoom({ isPaused: true })} activeHover={activeHover} onGuess={() => {}} isTextOnly={false} /> ) : <GuesserView timeLeft={roomData.timeLeft} describerName={currentP?.name} describerTeam={roomData.teamNames[currentP?.teamIdx]} isTeamMode={roomData.gameMode === 'team'} totalScores={roomData.totalScores} roundScore={roomData.roundScore} entities={gameTargets} onPause={() => updateRoom({ isPaused: true })} /> )}
